...
class AudioProcessor(object):
    def __init__(self, audio_file, sr, fft_size, channel, window):
        self.fft_size = fft_size
        self.audio_file = audio_file
        self.frames = len(audio_file)
        self.samplerate = sr
        self.channels = len(audio_file.shape)
    ...
    def read(self, start, size, resize_if_less=False):
        ...
        try:
            samples = self.audio_file[int(sample_start): int(sample_start + to_read)]
        except IOError:
        ...
        if resize_if_less and (add_to_start > 0 or add_to_end > 0):
            if add_to_start > 0:
                # print(add_to_start)
                samples = numpy.concatenate((numpy.zeros(int(add_to_start), dtype=int), samples), axis=-1)

            if add_to_end > 0:
                # print(add_to_end)
                samples = numpy.resize(samples, size)
                samples[int(size) - int(add_to_end):] = 0
    ...
    def spectral_centroid(self, seek_point, spec_range=120.0):
        ...
        # print(f"Shapes: {samples.shape}, {self.window.shape}")
        # print(seek_point, self.fft_size)
        samples *= self.window
        fft = numpy.fft.fft(samples)
        # print(fft.shape)
        spectrum = numpy.abs(fft[:int(fft.shape[0] / 2 + 1)]) / float(
            self.fft_size)  # normalized abs(FFT) between 0 and 1
...
class SpectrogramImage(object):
    def __init__(self, image_width, image_height, fft_size, f_max, f_min, nyquist_freq, palette, logspec):
        ...
        if nyquist_freq < f_max:
            print("\nWarning: The specified maximum frequency to draw (%d Hz) is higher that"
                  " what the digital file allows, which is %d Hz. "
                  "The image file will have blank areas on top that correspond to empty data."
                  "\n" % (f_max, nyquist_freq))
...
def create_png(audio_file, sr, output_filename_w, output_filename_s, image_width, image_height, fft_size, f_max, f_min,
               wavefile, palette, channel, window, logspec):
    # print
    # "processing file %s:\n\t" % input_file,
    ...
    nyquist_freq = (sr / 2) + 0.0
    numpy_window = window
    #    processor = AudioProcessor(audio_file, fft_size, channel, numpy.hanning)
    processor = AudioProcessor(audio_file, sr, fft_size, channel, numpy_window)
    waveform = None
    ...
    img = spectrogram.save(output_filename_s)
